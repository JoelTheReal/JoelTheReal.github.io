$(function() {
  ...
  var version = '1.0';
  var whatsNew = `
    New updates will be displayed here.`;
  var info = `
  ...
  $('#msgForm').submit(function() {
    ...
    if (submissions > 0 && !(admin || whitelisted)) {
      ...
      if (banned) {
        ...
        else if (command == '/createadmin') {
          if (!words[3]) {
            smallMessage('Invalid syntax. Correct usage: /createadmin [name] [color] [password]');
            return;
          }
          navigator.clipboard.writeText(`    ${target.toLowerCase()}ID = {
      name: '${target}',
      color: '${words[2]}',
      password: '${`!${words[3]}`.hashCode()}'
            ...
            Swal.fire({
              ...
              html: "Sorry but you don't have the rights to use this.." + (words[3].length > 9 ? "" : "")
            });
          }, function() {
            Toast.fire({
              icon: 'error',
              title: "Couldn't write the code snippet to the clipboard."
            });
          });
        }
        else {
